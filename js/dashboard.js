const Dashboard = (() => {
  async function render(container) {
    ...
    const [clientsRes, sessionsRes, lastClientRes] = await Promise.all([
      ...
      db.from('sessions').select('id', { count: 'exact', head: true }).gte('date', firstDay),
      db.from('clients').select('prenom, nom, created_at').order('created_at', { ascending: false }).limit(1),
    ]);
    ...
    container.innerHTML = `
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card animate-in" style="animation-delay:.16s">
          ...
          <div class="stat-value stat-value--sm">${lastClient ? lastClient.prenom + ' ' + lastClient.nom : '—'}</div>
